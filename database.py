...
def init_db(force_reset=False):
    try:
        ...
        if force_reset:
            for tbl in ["user_documents", "user_skills", "users", "email_otps", "teams", "team_invites", "candidate_internship_approvals", "discovered_internships"]:
                try:
        ...
        try:
            ...
            print(f"[DB] users table init: {e}")
            conn.rollback()

        # Ensure career_intent column exists
        try:
        ...
        try:
            ...
            print(f"[DB] user_documents init: {e}")
            conn.rollback()
        ...
        # Candidate Internship Approvals Table
        try:
            cursor.execute(f"""
            ...
            print(f"[DB] candidate_internship_approvals init: {e}")
            conn.rollback()
        ...
        try:
            ...
            print(f"[DB] team_invites init: {e}")
            conn.rollback()
        ...
        print(f"[DB WARN] Database schema initialization notice: {e}")
